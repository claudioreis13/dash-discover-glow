        ...
        <ScrollArea className="flex-1 -mr-2 pr-2 max-h-72">
          <ul className="space-y-3">
            {items.slice(0, 8).map((p) => {
              ...
              return (
                <li
                  key={`${p.fornecedorId}-${p.numero}`}
                  className="flex items-start justify-between gap-3 p-3 rounded-lg border border-border/60 bg-card hover:bg-accent/30 transition-colors"
                >
